...
const TrackerTable = ({ type, tableFields, loading, deleteField }: { type: string; tableFields: any[]; loading: boolean, deleteField: (id: string) => void }) => {
        ...
        {loading ? (
          ...
          <table className="w-full text-sm text-left rtl:text-right">
            ...
            <tbody className="overflow-scroll">
              {tableFields.map((transaction: any) => (
                <tr key={transaction._id} className="odd:bg-white">
                  {columns.map((col, idx) =>  (
                    <td key={idx} className="px-6 py-4">
                      {col === "category" ? transaction[col]?.name : col === "type" ? transaction[col] : transaction[col]}
                    </td>
                  ))}
